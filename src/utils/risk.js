...
export function computeRisk(answers) {
  const scores = Object.values(answers).map(v => Number(v));
...
// ✅ Nueva función para enviar resultados al backend
export async function sendResultToBackend(result) {
  try {
    const response = await fetch("http://localhost:8000/api/resultados", {
      ...
      body: JSON.stringify({
        ...
        promedio: result.avg.toString(),
      }),
    });

    if (!response.ok) {
      console.error("Error al enviar resultado:", response.statusText);
    } else {
